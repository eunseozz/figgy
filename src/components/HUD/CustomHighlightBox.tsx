...
const CustomHighlightBox = () => {
  ...
  const borderStyle = useHUDStore((state) => state.borderStyle as BorderStyle);
...
const ColumnGroup = styled.div<{ $gap?: string }>`
  display: flex;
  flex-direction: column;
  gap: ${({ $gap }) => $gap || "8px"};
`;
...
const BorderStyleBox = styled.div<{ $style: BorderStyle; $selected: boolean }>`
  width: 100%;
  height: 28px;
...
const PreviewBox = styled.div<{ $bg: string; $border: string }>`
  width: 100%;
  height: 36px;
  border-radius: 6px;
