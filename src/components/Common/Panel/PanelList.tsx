            ...
            {items.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  <LabelText $isActive={!!item.isActive}>
                    {item.label}
                  </LabelText>
                </PanelListButton>
                <ButtonWrapper>
...
const ProjectList = styled.ul<{ $isOpen: boolean }>`
  margin-top: 12px;
  overflow-y: auto;
...
const IconButton = styled.button<{ $isActive: boolean }>`
  ...
  svg {
    font-size: 16px;
    color: ${({ $isActive }) => ($isActive ? "#6ebbbf" : "#aaa")};
  }
`;

const PanelListButton = styled.button<{ $isActive: boolean }>`
  display: flex;
  width: 100%;
  height: 40px;
  gap: 14px;
