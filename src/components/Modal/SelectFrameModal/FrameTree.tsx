...
type FrameTreeProps = {
  ...
  onGroupToggle: (node: FigmaNode, checked: boolean) => void;
  onFrameToggle: (
    id: string,
    checked: boolean,
    parent: FigmaNode | null,
        ...
        return (
          ...
          >
            <Checkbox
              checked={!!checkedMap[node.id]}
              onChange={(e) =>
                onFrameToggle(node.id, e.target.checked, parent ?? null)
              }
            />
            <PageIcon />
...
const GroupHeader = styled.div<{ $depth: number }>`
  display: flex;
  align-items: center;
  gap: 8px;
  font-weight: 500;
...
const TreeFrame = styled.div<{ $depth: number }>`
  display: flex;
  align-items: center;
  gap: 8px;
  padding-left: ${({ $depth }) => ($depth === 0 ? 0 : $depth * 24)}px;
