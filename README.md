...
- **[개발 과정](#-개발-과정)**
    - [1. 구조가 다른 Figma와 DOM을 비교하기 위한 기준 설계](#1-구조가-다른-figma와-dom을-비교하기-위한-기준-설계)
        ...
        - [1-4. 일치 기준 오차 설정과 무시 조건 처리](#1-4-일치-기준-오차-설정과-무시-조건-처리)
...
<br/>
위의 문제의식을 바탕으로 **Figma 디자인을 구현 화면 위에 오버레이**하고, **시각적으로 차이를 확인할 수 있는 확장 프로그램**을 기획하게 되었습니다.
...
사용자는 오버레이된 디자인 이미지 위에서 DOM 요소를 클릭하면 해당 요소가 디자인과 얼마나 일치하는지를 툴팁 형태로 확인할 수 있습니다.<br/>
<br/>
...
- 렌더링 크기도 달라지므로 디자이너가 1440px 기준으로 만든 디자인도 브라우저에서는 화면 너비에 따라 1080px로 축소되어 보일 수 있습니다.
...
예를 들어, 위 내용과 같은 케이스의 경우 Figma에서 제공하는 좌표값에 × 0.75의 스케일 비율을 적용해야 실제 브라우저 상의 위치와 일치합니다.<br/>
...
사용자가 DOM 요소를 클릭하면 해당 요소가 디자인과 얼마나 어긋나 있는지를 방향과 거리로 직관적으로 보여주는 툴팁이 표시됩니다.<br/>
<br/>
**하지만 이 기능은 단순히 “몇 px 어긋났다”는 메시지를 띄우는 문제를 넘어 툴팁이 항상 정확히 해당 요소에 붙어 있도록 계산하고 화면 리사이즈나 스크롤 같은 상황에서도 어긋나지 않게 유지해야 하는 복잡한 구현 과제가 있었습니다.**
...
| 요소 위치     | `event.target.getBoundingClientRect()`   | 요소의 브라우저 내 상대 좌표 (`top`, `left`, `width`, `height`) 추출 |
| 기준점 보정   | `rect.top + scrollY`, `rect.left + scrollX` | 뷰포트 기준 좌표를 페이지 전체 기준 절대 좌표로 보정               |
...
| 항목         | 값                                              | 설명                                                             |
|--------------|--------------------------------------------------|------------------------------------------------------------------|
| 기준 위치     | `rect.top + scrollY`, `rect.left + scrollX`      | 뷰포트 기준 위치에 스크롤 보정을 더해 절대 위치 계산              |
...
<br/>
예를 들어, 오차가 감지되더라도 그 정보가 언제 어떻게 보여지느냐에 따라 사용자가 느끼는 이해도나 몰입도가 크게 달라질 수 있다는 생각이 들었고<br/>
...
<br/>
이번 프로젝트는 제가 UX를 처음으로 ‘마지막에 덧붙이는 요소’가 아니라, ‘기획의 가장 앞단에서부터 함께 고민해야 하는 핵심’으로 받아들인 경험이었습니다.<br/>
